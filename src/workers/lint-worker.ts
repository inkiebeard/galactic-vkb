/**
 * Lint worker — long-running polling loop (mirrors finetune-worker pattern).
 * Claims queued lint jobs one at a time using SELECT FOR UPDATE SKIP LOCKED.
 * Spawned once at startup by coordinator.
 */
import { getPool, isDbConnectionError } from '../db/client.js';
import { getAdapters } from '../adapters/registry.js';
import { runLintPipeline } from '../pipeline/lint.js';
import { createLogger } from '../logger.js';

const log = createLogger('lint-worker');
...
async function claimJob(): Promise<string | null> {
  ...
  try {
    ...
    const { rows } = await client.query<{ id: string }>(
      `SELECT id FROM job
       WHERE kind = 'lint' AND stage = 'queued'
    ...
    await client.query(
      `UPDATE job SET worker_pid = $1, stage = 'checking_orphans' WHERE id = $2`,
      [process.pid, jobId],
    );
...
async function main(): Promise<void> {
  ...
  while (true) {
    try {
      ...
      log.info(`Claimed lint job ${jobId}`);
